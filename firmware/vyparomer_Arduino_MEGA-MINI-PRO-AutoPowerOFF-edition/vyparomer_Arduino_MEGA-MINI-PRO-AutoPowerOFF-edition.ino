...
/*-----------------------------------------------------------------------------------------------------------------*/
/* Intenzita slunečního záření - senzor proudu INA219
  PINOUT:
  PINOUT:
  VIN - 5V
  GND - GND
...
#define ADDR 0x40 // definovani adresy senzoru INA219
Adafruit_INA219 ina219(ADDR); // inicializace senzoru s nastavenou adresou z knihovny
float ShortCircuitCurrentSTC = 0.435   ;    // STC - maximalni vystupni zkratovy proud solarniho clanku dle datasheetu
/*-----------------------------------------------------------------------------------------------------------------*/
...
void setup()
{
  ...
  //setTime(15, 05, 00, 23, 11, 2021); //nastaveni casu format H, M, S, d, m, y
  ...
  ina219.begin(); // inicializace INA219
...
void nactiSenzory() {
  ...
  float SolarIrradiance = ((ina219.getCurrent_mA() + 0.1) / ShortCircuitCurrentSTC); //intenzita slunecniho zareni [W/m2], 0.1 je kompenzace proudu z Arduina
  
  zapisData(temp, hum, pres, dpth, wtemp, DStemps, SolarIrradiance);
...
void zapisData(float temp, float hum, float pres, float dpth, float wtemp, String DStemps, float SolarIrradiance) {
  ...
  vysledkyMereni = String(year(casMereni)) + "/" + String(month(casMereni)) + "/" + String(day(casMereni)) + ";" + String(hour(casMereni)) + ":" + String(minute(casMereni)) + ":" + String(second(casMereni)) + ";" + String(temp) + ";" + String(hum) + ";" + String(pres) + ";" + String(dpth) + ";" + String(wtemp) + ";" + DStemps+ ";" + String(SolarIrradiance);
